\documentclass[mscthesis, 11pt]{usiinfthesis}
\usepackage{lipsum}
\usepackage[T1]{fontenc}
\usepackage[utf8]{inputenc}
\usepackage[italian, english]{babel}
\usepackage{datenumber}
\usepackage{minitoc}        % TODO See why it does not work
\usepackage{todonotes}
\renewcommand{\baselinestretch}{1.2}
\renewcommand*{\bibfont}{\footnotesize} % \small
\usepackage{graphics}
\usepackage{graphicx}
\usepackage{booktabs}
\usepackage{subcaption}
\usepackage{float}
\graphicspath{{images/}}
\usepackage{enumitem}
\setlist[1]{itemsep=2pt}
\usepackage{acronym}

\usepackage{color,soul}
\definecolor{mygray}{RGB}{242,242,242}
\definecolor{mydarkgray}{RGB}{193,193,193}
\definecolor{myblue}{RGB}{80, 100, 150}
\definecolor{mygreen}{RGB}{80, 160, 60}

\usepackage{verbatim}
\usepackage{listings}
\lstset{language=python,
	frame=single,
    frameshape={RYR}{Y}{Y}{RYR},
    rulecolor=\color{mydarkgray},
    backgroundcolor=\color{white},
    keywordstyle=\color{myblue},
    commentstyle=\color{mygreen},
    tabsize=3,
    basicstyle=\small\ttfamily,
    numbers=left,
    firstnumber=1,
    numberfirstline=true,
    numberstyle=\tiny\color{mydarkgray},
    breaklines=true,
    postbreak=\mbox{\textcolor{mydarkgray}{$\hookrightarrow$}\space}
}

\lstdefinelanguage{algebra}
{morekeywords={import,sort,constructors,observers,transformers,axioms,if,
else,end},
sensitive=false,
morecomment=[l]{//s},
}


\title{Prediction of Epileptic Seizures using\\Machine Learning and Deep Learning models} %compulsory
% \title{Prediction of Epileptic Seizures\\with Graph-based Deep Learning} %compulsory
%\subtitle{Subtitle: Reinventing the World} %optional 
\author{Alessia Ruggeri} %compulsory
\specialization{Artificial Intelligence}%optional
\begin{committee}
\advisor{Prof.}{Cesare}{Alippi} %compulsory
\coadvisor{Dr.}{Daniele}{Grattarola}{} %optional
\end{committee}
\Day{5th} %compulsory
\Month{September} %compulsory
\Year{2019} %compulsory, put only the year
\place{Lugano} %compulsory

%\dedication{To my beloved} %optional
%\openepigraph{Someone said \dots}{Someone} %optional

%\makeindex %optional, also comment out \theindex at the end

\begin{document}

%************************************************
% INITIAL SECTION
%************************************************
\maketitle %generates the titlepage, this is FIXED
\frontmatter %generates the frontmatter, this is FIXED

\input{corpus/abstract}\clearpage
\input{corpus/acknowledgements}\clearpage
\tableofcontents
\input{corpus/acronyms}\clearpage
% \listoffigures %optional
% \listoftables %optional

%************************************************
% MAIN SECTION
%************************************************
\mainmatter

\input{chapters/1_introduction}\clearpage
\input{chapters/2_background}\clearpage
\input{chapters/3_methods}\clearpage
\input{chapters/4_implementation}\clearpage
\input{chapters/5_conclusion}\clearpage
% \input{chapters/random-stuff.tex}\clearpage   KEEP COMMENTED
% \input{corpus/appendix}\clearpage     KEEP COMMENTED


%************************************************
% FINAL SECTION
%************************************************
\backmatter

% \nocite{*}
\bibliographystyle{unsrtnat} %plainnat
\bibliography{corpus/biblio}
%\chapter{Glossary} %optional
% \input{corpus/bibliography}\clearpage


%\cleardoublepage
%\theindex %optional, use only if you have an index, must use
%\makeindex in the preamble

\end{document}



% BIBLIOGRAPY STUFF as example

% @article{einstein,
%     author =       "Albert Einstein",
%     title =        "{Zur Elektrodynamik bewegter K{\"o}rper}. ({German})
%         [{On} the electrodynamics of moving bodies]",
%     journal =      "Annalen der Physik",
%     volume =       "322",
%     number =       "10",
%     pages =        "891--921",
%     year =         "1905",
%     DOI =          "http://dx.doi.org/10.1002/andp.19053221004"
% }
 
% @book{latexcompanion,
%     author    = "Michel Goossens and Frank Mittelbach and Alexander Samarin",
%     title     = "The \LaTeX\ Companion",
%     year      = "1993",
%     publisher = "Addison-Wesley",
%     address   = "Reading, Massachusetts"
% }
 
% @misc{knuthwebsite,
%     author    = "Donald Knuth",
%     title     = "Knuth: Computers and Typesetting",
%     url       = "http://www-cs-faculty.stanford.edu/\~{}uno/abcde.html"
% }